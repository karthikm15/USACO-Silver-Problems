...
int main() {
    ...
    while (true) {
        ...
        //cout << "THE SEATING IS " << seating << endl;
        int placeholder=0;
        placeholder = arr[seating-1];
        arr[seating-1] = cow;
        cow = placeholder;
        //cout << "THE NEW COW IS " << cow << endl;
        bool isDone = true;
        for (vector<int>:: iterator i = alreadymoved.begin(); i != alreadymoved.end(); i++) {
            if (cow == (*i)) {
                cout << theone;
                isDone = false;
                break;

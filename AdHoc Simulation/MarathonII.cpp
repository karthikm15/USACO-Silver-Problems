...
int main() {
    ...
    long long int minimum = 10000000000;
    long long int finaldistance = 0;
    for (int i = 1; i < n; i++) {
        finaldistance +=  abs(arr[i].x - arr[i-1].x) + abs(arr[i].y - arr[i-1].y);
    }
    for (int i = 1; i < (n-1); i++) {
        long long int distance = 0;
        distance = finaldistance - (abs(arr[i].x - arr[i-1].x) + abs(arr[i].y - arr[i-1].y)) - (abs(arr[i].x - arr[i+1].x) + abs(arr[i].y - arr[i+1].y));
        distance += abs(arr[i+1].x - arr[i-1].x) + abs(arr[i+1].y - arr[i-1].y);
        if (minimum > distance) {
            minimum = distance;
        }
    }
